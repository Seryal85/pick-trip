(function(){
    ...
    return {
      ...
      link: function(scope, element, attrs, ngModel) {
        ...
        var weakest = dots.slice(0,1)[0];
        var weak = dots.slice(0,2);
        var strong = dots.slice(0,3);
        var strongest = dots.slice(0,4);
          ...
          if (ngModel.$viewValue) {
            ...
            if (strength > 85) {
              angular.forEach(strongest, function(el) {
                el.style.backgroundColor = '#60beb3';
              });
            } else if (strength > 65) {
              ...
              weakest.style.backgroundColor = '#60beb3';
            }
          }
        });
      },
      template: '<div class="password-strength-indicator"><div></div><div></div><div></div><div></div></div>'
    };
  })
  ...
  .controller('SignUpCtrl', function($scope, $location, $auth, $log) {
    $scope.signUp = function() {
      $auth.signup($scope.user)
        .then(function(response) {
          $auth.setToken(response);
          $location.path('/');
          $log.info('You have successfully created a new account and have been signed-in');
        })
